...
posteditops=list() # empty postediting operator list, to be filled after all sentences are processed
...
output=args.output
if output:
  pe_out = open(args.output, 'w')
  for s,t,ref in posteditops :
    if output :
       pe_out.write( "\"" + s + "\",\"" +   t + "\",\"" + ref + "\"\n" )
    else :
       print(  "\"" + s + "\",\"" +   t + "\",\"" + ref + "\"\n" )
if output :
   pe_out.close()
